        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{__('menu.Create Menu')}}</h3>
                    <div class="card-tools">
                        <a href="{{ route('menus.index') }}" title="{{__('menu.Menus List')}}" class="btn btn-success btn-sm">
                            <i class="fas fa-list"></i> {{__('menu.Menus List')}}
                        </a>
                    </div>
                </div>
                ...
                <form role="form" id="create-form" name="create-form" method="post" action="{{ route('menus.store') }}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="title" placeholder="{{ __('menu.name') }}" value="{{ old('name') }}" required>
                                @error('name')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug" id="slug" placeholder="{{ __('menu.slug') }}" value="{{ old('slug') }}" required>
                                @error('slug')
                                <div class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select name="parent_id" id="parent_id" class="form-control select2 select2-success @error('parent_id') is-invalid @enderror" data-dropdown-css-class="select2-success">
                                    {{--<option value="">{{__('shared.NA')}}</option>
                                    @foreach ($menu->getItemsByParent() as $collection)
                                        <option value="{{ $collection->getKey() }}">{{ $collection->name }}</option>
                                    @endforeach--}}
                                    {!! $menu->toSelect($menu->getTree(), 0, old('parent_id')) !!}
                                </select>
                                @error('parent_id')
                                <div class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description" rows="5" placeholder="{{ __('menu.description') }}">{{ old('description') }}</textarea>
                                @error('description')
                                <div class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <div class="icheck-success icheck-inline">
                                    ...
                                    <label class="form-check-label" for="is_enable1">{{__('shared.active')}}</label>
                                </div>
                                <div class="icheck-success icheck-inline">
                                    <input type="radio" name="is_enable" id="is_enable0" class="form-check-input @error('is_enable') is-invalid @enderror" value="0">
                                    <label class="form-check-label" for="is_enable0">{{__('shared.inactive')}}</label>
                                </div>
                                @error('is_enable')
                                <div class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
